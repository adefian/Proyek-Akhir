          ...
          <div class="row">
            ...
            <div class="col-lg-6 col-sm-12 col-12">
                @if(auth()->user()->role == 'pimpinanecoranger')
                    <form id="editForm" class="needs-validation" novalidate="" action="daftarkomunitas" method="POST" enctype="multipart/form-data">
                @endif
                @if(auth()->user()->role == 'petugaslapangan')
                    <form id="editFormpetugaslap" class="needs-validation" novalidate="" action="daftarkomunitas-petugaslap" method="POST" enctype="multipart/form-data">
                @endif
                @if(auth()->user()->role == 'komunitas')
                    <form id="editFormkomunitas" class="needs-validation" novalidate="" action="daftarkomunitas-komunitas" method="POST" enctype="multipart/form-data">
                @endif
                    {{csrf_field()}}
                    {{ method_field('PATCH') }}
