...
for(var i = 0; i < chats.length; i++) {
  ...
  $.chatCtrl('#mychatbox', {
    ...
    picture: (chats[i].position == 'left' ? '../foto_user/avatar-1.png' : '../foto_user/avatar-2.png'),
...
$("#chat-form").submit(function() {
  ...
  if(me.find('input').val().trim().length > 0) {
    $.chatCtrl('#mychatbox', {
      ...
      picture: '../foto_user/avatar-2.png',
    });
    me.find('input').val('');
  }
  return false;
...
for(var i = 0; i < chats.length; i++) {
  ...
  $.chatCtrl('#mychatbox2', {
    text: (chats[i].text != undefined ? chats[i].text : ''),
    picture: (chats[i].position == 'left' ? '../foto_user/avatar-5.png' : '../foto_user/avatar-2.png'),
    position: 'chat-'+chats[i].position,
    type: type
  });
}
